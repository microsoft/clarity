...
export function set(variable: string, value: string | string[]): void {
    const values = typeof value === Constant.String ? [value as string] : (value as string[]);
    log(variable, values);
}
...
function log(variable: string, value: string[]): void {
    if (core.active() && variable && value && typeof variable === Constant.String && variable.length < 255) {
        const validValues = variable in data ? data[variable] : [];
        for (let i = 0; i < value.length; i++) {
            if (typeof value[i] === Constant.String && value[i].length < 255) {
