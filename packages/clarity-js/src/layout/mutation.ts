...
import { Constant, type MutationHistory, type MutationQueue, type MutationRecordWithTime, Setting, Source } from "@clarity-types/layout";
...
function track(m: MutationRecord, timer: Timer, instance: number, timestamp: number): string {
    ...
    if (value && value.data.tag !== Constant.HTML) {
        ...
        const element = target && target.selector ? target.selector.join() : m.target.nodeName;
