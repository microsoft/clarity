...
export function parse(root: ParentNode, init = false): void {
    ...
    try {
        ...
        if (init) {
            config.unmask.forEach((x) => (x.indexOf(Constant.Bang) < 0 ? unmask.push(x) : override.push(x.substr(1))));
        }
...
function privacy(node: Node, value: NodeValue, parent: NodeValue): void {
    ...
    switch (true) {
        ...
        case tag === Constant.TextTag:
            ...
            const pTag = parent && parent.data ? parent.data.tag : Constant.Empty;
            const pSelector = parent && parent.selector ? parent.selector[Selector.Default] : Constant.Empty;
