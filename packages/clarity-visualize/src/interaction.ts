...
export function pointer(event: Interaction.PointerEvent, iframe: HTMLIFrameElement): void {
    ...
    if (p === null) {
        ...
        p.style.zIndex = `${config.zIndex + 2}`;
        ...
        style.innerText = "@keyframes pulsate-one { 0% { transform: scale(1, 1); opacity: 1; } 100% { transform: scale(3, 3); opacity: 0; } }";
        style.innerText += "@keyframes pulsate-two { 0% { transform: scale(1, 1); opacity: 1; } 100% { transform: scale(5, 5); opacity: 0; } }";
        p.appendChild(style);
    }

    p.style.left = (data.x - 5) + "px";
    p.style.top = (data.y - 1) + "px";
    switch (type) {
...
function drawClick(doc: Document, x: number, y: number): void {
    ...
    click.style.zIndex = `${config.zIndex + 1}`;
    click.style.left = (x - config.clickWidth / 2) + "px";
    click.style.top = (y - config.clickHeight / 2) + "px"
    click.style.width = config.clickWidth + "px";
    click.style.height = config.clickHeight + "px";
    click.style.borderRadius = "50%";
    click.style.background = "radial-gradient(rgba(255,0,0,0.8), transparent)";
    doc.body.appendChild(click);

    // First pulsating ring
    let ringOne = click.cloneNode() as HTMLElement;
    ringOne.style.left = "-0.5px";
    ringOne.style.top = "-0.5px";
    ringOne.style.background = "transparent";
    ringOne.style.border = "1px solid rgba(255,0,0,0.8)";
...
function overlay(iframe: HTMLIFrameElement): HTMLCanvasElement {
    ...
    let canvas = doc.getElementById("clarity-canvas") as HTMLCanvasElement;
    if (canvas === null) {
        canvas = document.createElement("canvas");
        canvas.id = "clarity-canvas";
        canvas.width = 0;
        canvas.height = 0;
