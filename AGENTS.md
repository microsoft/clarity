...
See [ARCHITECTURE.md](./ARCHITECTURE.md) for a full architecture overview with diagrams covering the package dependency chain, data flow, module structure, build variants, privacy system, and performance techniques.
...
2. **Test failures:** Ensure build artifacts exist with `yarn build` before running tests, and that Playwright's browser is installed: `npx playwright install chromium`
...
When in doubt, refer to the source — the codebase is the ground truth.
